...
import SANGEBOTS
from SANGEBOTS.lib.curve.ttypes import *
...
║Gcancelall
║Ginfo
║Sange in (Masukin bot)
║Sange out (Keluarin bot)
║Setlastpoint
║Cctv
║Glink
...
wait = {
    ...
    'timeline':False,
    'autoAdd':True,
    'message':"""❂••••AUTO ADD BY  SANGEBOTS••••❂""",
    "lang":"JP",
    "comment1":"❂••••AUTO ADD BY SANGEBOTS••••❂",
...
def bot(op):
    try:
        ...
        if op.type == 26:
            ...
            if msg.toType == 0:
                msg.to = msg.from_
                if msg.from_ == "ucf8ce5f98ebb1963efb270488f5d8416":
                    if "join:" in msg.text:
        ...
        if op.type == 25:
            ...
            if msg.contentType == 13:
                ...
                msg.contentType = 13
                msg.contentMetadata = {'mid': 'ucf8ce5f98ebb1963efb270488f5d8416'}
                acil.sendMessage(msg)
...
#                else:
                ...
                if len(string.decode('utf-8')) <= 20:
                    ...
                    acil.sendText(msg.to,"􀜁􀇔􏿿semua nama sangebots telah di update menjadi\n👉 " + string + "👈")
...
#--------------------------------------------------------
                ...
                acil.sendText(msg.to,"❂MANTAP ANJENG❂")
                msg.contentType = 13
                msg.contentMetadata = {'mid': 'ucf8ce5f98ebb1963efb270488f5d8416'}
                acil.sendMessage(msg)
                acil.sendText(msg.to,"❂•••••{CREATOR SANGEBOTS}•••••❂")
                ...
                msg.contentType = 13
                msg.contentMetadata = {'mid': 'ucf8ce5f98ebb1963efb270488f5d8416'}
                acil.sendText(msg.to,"❂OPEN GRUP LENDIR❂")
                acil.sendMessage(msg)
                acil.sendText(msg.to,"❂LANGSUNG DI GAS❂")
                ...
                for i in gid:
                    ...
                    ki6.leaveGroup(i)
                if wait["lang"] == "JP":
                    acil.sendText(msg.to,"SANGEBOT Sudah Keluar Di semua grup")
                else:
...
#-----------------------------------------------------------
                    ...
                    acil.sendText(msg.to, "Check Yang sange")
                    try:
...
#-----------------------------------------------
#-----------------------------------------------
            elif msg.text.lower() == 'sange out':
                if msg.toType == 2:
                    ginfo = acil.getGroup(msg.to)
                    try:
                        acil.sendText(msg.to,"􀜁􀇔􏿿Bye Bye😘 minggat "  +  str(ginfo.name)  + "")
                        pb1.leaveGroup(msg.to)
                        pb2.leaveGroup(msg.to)
                    except:
                        pass
#-----------------------------------------------
            elif "itil bye" in msg.text:
                if msg.toType == 2:
                    ginfo = acil.getGroup(msg.to)
                    try:
                        pb1.leaveGroup(msg.to)
...
#-----------------------------------------------
                ...
                ginfo = acil.getGroup(msg.to)
                acil.sendText(msg.to,"Selamat Datang Di Neraka " + str(ginfo.name))
                ...
                pb1.sendText(msg.to,"hadir 􀜁􀇔􏿿")
                pb2.sendText(msg.to,"hadir 􀜁􀇔􏿿")
